...
TEST_F(StressTestServer, rbus_resolveWildcardDestination_test4)
{
    ...
    else if (pid > 0)
    {
        ...
        printf("@@@@@@@@@@@@@@@@@@@@@@@@@@server_obj1='%s'\n", server_obj1);
        printf("@@@@@@@@@@@@@@@@@@@@@@@@@@server_obj2='%s'\n", server_obj2);
        printf("@@@@@@ function %s line %d\n",__func__,__LINE__);

         // Order-independent check
        bool found1 = false, found2 = false;
        for (int j = 0; j < 2; ++j) {
            if (strncmp(result_array[j], server_obj1, MAX_ELEMENT_NAME_LENGTH) == 0)
                found1 = true;
            if (strncmp(result_array[j], server_obj2, MAX_ELEMENT_NAME_LENGTH) == 0)
                found2 = true;
        }
        EXPECT_TRUE(found1) << "server_obj1 not found in result_array";
        EXPECT_TRUE(found2) << "server_obj2 not found in result_array";
